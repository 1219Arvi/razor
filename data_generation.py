...
real data. See data_analysis.ipynb for the fitting work itself.
...
Last verified run (seed=42, n_orders=5000)
-------------------------------------------
...
Usage
-----
Everything that used to run at import time now lives inside
`generate_dataset()`. Importing this module no longer has side effects -
call the function (directly, or via `python data_generation.py`) to
actually generate and save data.
"""
...
# =======================================================================
# Fixed configuration (not re-fit per run - these describe the market
# model, not the random draw)
# =======================================================================
...
FULL_CSV = "data/interim/synthetic_orders_full.csv"
MODEL_CSV = "data/interim/synthetic_orders_model.csv"
...
def _default_n_customers(n_orders: int, target_mean_orders_per_customer: float = 7.55) -> int:
    """
    Sizes the customer pool so expected total orders (n_customers x mean
    orders per customer) lands near n_orders with no post-hoc trimming
    needed. An earlier version generated a larger pool and trimmed down -
    that was rejected because random trimming disproportionately erases
    low-order-count customers, distorting the fitted shape. Sizing the
    pool correctly up front avoids that problem entirely.
    """
    return max(1, round(n_orders / target_mean_orders_per_customer))
...
def generate_dataset(
    ...
    seed: int = 42,
    target_base_rate: float = TARGET_BASE_RETURN_RATE,
    verbose: bool = True,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Generates the synthetic return-risk dataset.

    Returns (df_full, df_model):
      - df_full includes the ground-truth `_latent_propensity` and
        `p_return_true` columns - keep for validation/debugging only.
      - df_model is what a model should actually be trained on (leakage
        columns dropped).

    This function has no side effects beyond using `rng` - it does not
    write any files. Call `save_dataset()` (or run this file as a script)
    to persist the CSVs.
    """
    rng = np.random.default_rng(seed)
    if n_customers is None:
        n_customers = _default_n_customers(n_orders)
    ...
    # binomial, build the order list by repeating customer indices that
    # many times, then shuffle. The pool was sized above so this rarely
    # needs more than a small trim/pad to land exactly on n_orders.
    ...
    def make_order_row(i: int) -> dict:
        ...
        # Left as NaN for first-time buyers - this is a deliberate,
        # realistic missing-data case a real model has to handle, not a
        # bug to patch.
        if past_orders > 0:
        ...
        return {
            ...
            # Ground-truth column, kept for validation only - dropped
            # before the model-training CSV is written.
            "_latent_propensity": latent_propensity,
        }

    df = pd.DataFrame(make_order_row(i) for i in range(n_orders))

    # --- Step 4: ground-truth risk label ---------------------------------
    # Combine weighted features into a logit, add noise, convert to a
    # probability, then sample the binary `returned` outcome. Weights
    # below are the deliberately injected "true" relationships - used
    # later to sanity-check that a trained model's feature importances
    # roughly recover them.
    order_value_norm = (df["order_value"] - df["order_value"].mean()) / df["order_value"].std()
    is_cod = (df["payment_method"] == "COD").astype(int)

    # First-time buyers have no observed past_return_rate (NaN by
    # design). Fill with the population mean so the formula doesn't
    # break on NaN - this mirrors what a real downstream model has to
    # do too.
    ...
    logit = (
        ...
        - 0.4 * order_value_norm                           # GROUNDED: real UCI data shows higher-
                                                            ...
                                                            # cancelled - sign flipped from the
    ...
    bias = find_bias_for_target_rate(logit.values, target_base_rate)
    if verbose:
        print(f"Calibrated bias: {bias:.4f} (target base rate {target_base_rate})")
    ...
    df_full = df
    df_model = df.drop(columns=["_latent_propensity", "p_return_true"])
    return df_full, df_model


def save_dataset(df_full: pd.DataFrame, df_model: pd.DataFrame, full_csv: str = FULL_CSV, model_csv: str = MODEL_CSV) -> None:
    for path in (full_csv, model_csv):
    ...
    # (ground-truth leakage columns already removed by generate_dataset).
    df_model.to_csv(model_csv, index=False)

    print("\nSaved:")
    print(f" - {full_csv}  (includes ground truth, for validation only)")
    print(f" - {model_csv} (use this for training)")


if __name__ == "__main__":
    df_full, df_model = generate_dataset()
    save_dataset(df_full, df_model)